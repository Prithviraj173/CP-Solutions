...
void solve() {
    ...
    for(auto &num : arr) maxi = max(maxi, *max_element(num.begin(), num.end()));
    vector<ll> count(maxi + 1, 0), r(n, 0), c(m, 0);
    for(ll i = 0; i <= maxi; i++) {
        ll x = i;
        while(!isPrime(x)) x++;
        count[i] = x - i;
    ...
    for(auto &num : r) ans = min(ans, num);
    for(auto &num : c) ans = min(ans, num);
    cout << ans << endl;
}

...
void solve() {
    ...
    vector<ll> res(n + 1);
    for(ll i = 0; i < n; i++) res[arr[i]] = i;
    vector<ll> dp(n + 2, 0);
    dp[1] = 1;
    for(ll i = 2; i <= n; i++) {
        vector<ll> nxt(i + 1, 0);
        vector<ll> pref(i, 0);
        for(ll j = 1; j < i; j++) pref[j] = (pref[j - 1] + dp[j]) % M;
        bool flag = (res[i - 1] < res[i]);
        for(ll j = 1; j <= i; j++) {
            if(flag) nxt[j] = pref[j - 1];
            else nxt[j] = (pref[i - 1] - pref[j - 1] + M) % M;
        }
        dp = move(nxt);
    }
    ll ans = 0;
    for(ll i = 1; i <= n; i++) ans = (ans + dp[i]) % M;
    cout << ans << endl;
}
